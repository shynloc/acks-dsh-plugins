...
window.__ModuleLoader__.load({
  ...
  factory: (require) => {
    ...
    var API_KEY_REF = "MIMO_API_KEY";
    var BASE_URL_REF = "MIMO_BASE_URL";
    var PAYG_BASE_URL = "https://api.xiaomimimo.com/v1";
    ...
    function normalizeBaseUrl(value) {
      ...
      var allowed = hostname === "api.xiaomimimo.com" ||
        /^token-plan-[a-z0-9-]+\.xiaomimimo\.com$/.test(hostname);
      var pathname = url.pathname.replace(/\/+$/, "");
      if (url.protocol !== "https:") throw new Error("Base URL 必须使用 https://");
      if (url.username || url.password || (url.port && url.port !== "443")) {
        throw new Error("Base URL 不得包含用户信息或非标准端口");
      }
      if (url.search || url.hash) throw new Error("Base URL 不得包含查询参数或片段");
      if (!allowed || pathname !== "/v1") {
        throw new Error("仅支持小米按量地址或 token-plan-*.xiaomimimo.com/v1");
      }
    ...
    function MimoVisionCard(props) {
      ...
      return h("li", { style: { padding: "12px 0" } },
        h("div", { style: { display: "flex", alignItems: "center", flexWrap: "wrap", gap: 8, marginBottom: 4 } },
          h("span", { style: { fontSize: 13, fontWeight: 600 } }, "mimo-vision（MiMo 视觉理解）"),
          statusBadge(keyView && keyView.configured ? "API Key 已配置" : "API Key 未配置", Boolean(keyView && keyView.configured)),
          statusBadge(baseView && baseView.configured ? "Base URL 已配置" : "使用按量默认地址", true)),
        h("p", { style: { margin: "0 0 12px", fontSize: 12, color: "var(--dsw-alias-label-tertiary, #888)" } },
          "按量 Key（sk-）可使用默认地址；Token Plan Key（tp-）必须同时保存套餐页面提供的 OpenAI 兼容 Base URL。两个值均通过 DSH 凭据服务写入并且不会回显。"),

        h("label", { htmlFor: "mimo-api-key", style: { fontSize: 13, fontWeight: 500, display: "block", marginBottom: 4 } },
        ...
        input({
          ...
          placeholder: keyView && keyView.configured ? "••••••••（已配置）" : "sk-... 或 tp-...",
          value: keyDraft,
        ...
        h("label", { htmlFor: "mimo-base-url", style: { fontSize: 13, fontWeight: 500, display: "block", marginBottom: 4 } },
          "Base URL" + (baseView && baseView.configured ? "（输入新值可替换）" : "（未配置时仅按量 Key 使用默认地址）")),
        input({
          id: "mimo-base-url",
          type: "url",
          list: "mimo-base-url-presets",
          autoComplete: "off",
          spellCheck: false,
          disabled: saving || !baseWritable,
          placeholder: baseView && baseView.configured ? "已配置；输入新 URL 可替换" : PAYG_BASE_URL,
          value: baseUrlDraft,
          onChange: function (event) { setBaseUrlDraft(event.target.value); }
        }),
        h("datalist", { id: "mimo-base-url-presets" },
        ...
        h("p", { style: { margin: "5px 0 8px", fontSize: 11, color: "var(--dsw-alias-label-tertiary, #888)" } },
          "中国 Token Plan 示例：https://token-plan-cn.xiaomimimo.com/v1；请以套餐页面显示为准。"),
        h("div", { style: { display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" } },
          ...
          baseView && baseView.configured
            ? button("清除 Base URL", { disabled: saving || !baseWritable, onClick: function () { onClear(BASE_URL_REF, "Base URL 已清除；按量 Key 将恢复默认地址"); } })
            : null,
